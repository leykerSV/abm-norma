...
class Norma_controller extends CI_Controller {
    ...
    public function editar() {
        ...
        $this->load->view('seleccion',$datos);
    }
    ...
    public function buscar() {
        ...
        if($this->form_validation->run()){ 
                ...
                $datos['tiponorma'] = $this->Norma_model->get_tiponorma();
                $datos['listaNormas']=$this->Norma_model->get_norma1($_POST['numnorma'],$_POST['tiponorma']);
                $this->load->view('seleccion',$datos);    
    ...
    public function guarda_norma() {
        ...
        if($this->form_validation->run()){ 
            ...
            if ($_FILES['archivoord']['name']==null){
                $nombrearchivoord=$_POST['archivoord1'];	 	       
            }else{
                $fichero_subido = $directorio . basename($_FILES['archivoord']['name']);
                $nombrearchivoord=basename($_FILES['archivoord']['name']);
                move_uploaded_file($_FILES['archivoord']['tmp_name'], $fichero_subido);
            ...
            $resp1 = $this->Norma_model->guarda_norma($_POST);
            echo 'Norma Modificada con exito';
            echo '<br />';
            echo '<a href="https://abm-norma.concejosantotome.gob.ar/index.php/abmnorma">Volver al inicio de Carga</a>';
        }else{
            $this->index();
